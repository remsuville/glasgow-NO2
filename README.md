...
- **Fetches** daily NO₂ column density from CDSE openEO (`SENTINEL_5P_L2`) for a
...
- **Aggregates onto administrative zones** — council areas, Intermediate Zones
  2022, or Data Zones 2022 — by area-weighted overlap, switchable live.
- **Reports its own uncertainty.** Every zone carries a `coverage` fraction, and
  zones backed by too little valid data are drawn grey rather than blindly calculated.
...
of the map. Each zone's mean is instead `Σ(area·NO₂) / Σ(area)` over every
...
not a constant area — at 56°N a degree of longitude spans ~56% of a degree of
latitude — so weighting by degree-area would systematically over-weight
whatever sits furthest north.
...
inside a Scotland envelope — which turns that silent failure loud in case it happens.

**Coverage travels with the mean.** A zone straddling the study bbox can be
handed a confident-looking number backed by a sliver of its area. i.e. Perth and
...
Then open <http://127.0.0.1:8050>. The first data request triggers a device-flow
...
!NOT COMMITTED! Download the sets you want from:
...
That last column is the one to read before choosing a set — see
...
average NO₂ over open water the zone has no population in).
...
data.py         CDSE openEO → NetCDF → DataFrame → parquet     (the only network I/O)
boundaries.py   shapefile → validate → reproject → parquet     (CRS)
zonal.py        pixels + zones → area-weighted means           (the analytical core)
...
Fuller write-up, including the cached data shapes and design rationale, in
...
- **Tropospheric column ≠ ground concentration.** Output is mol/m² through the
  ...
  Converting requires boundary-layer assumptions not made here.
...
- **Sub-pixel zones.** Council areas are the only set genuinely coarser than the
  ...
  means. The finer sets are a presentation choice, NOT added spatial detail!!
...
- [ ] Before/after comparison when swapping boundary sets — quantify what
      changes, not just redraw the map
